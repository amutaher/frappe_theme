				...
				if (frappe.user_roles.includes(d.allowed) && has_approval_access(d)) {
					...
					me.frm.page.add_action_item(__(d.action), async function () {
						...
						if (wf_dialog_fields?.length) {
							fields = me.frm.meta?.fields.filter(field => { return wf_dialog_fields.some(f => f.fieldname == field.fieldname) })
								.map(field => {
							...
							fields = me.frm.meta?.fields?.filter(field => {
                                ...
                                let dailog = new frappe.ui.Dialog({
                                    ...
                                    primary_action: (values) => {
                                        ...
                                        frappe.xcall("frappe.model.workflow.apply_workflow", {
                                            doc: { ...me.frm.doc, wf_dialog_fields: values ? values : {} },
                                            action: action
                                        }).then((doc) => {
