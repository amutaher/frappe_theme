    ...
    custom_setup_fields: function (frm, cdt, cdn) {
        ...
        frappe.model.with_doctype(doctype, () => {
                ...
                html += `
                    ...
                    ">
                        ...
                        ">
                            ...
                            >
                            <span class="field-label" style="font-size: 13px;">${field.label}</span>
                        </label>
                ...
                fields: [
                    {
                        ...
                        onchange: function() {
                            ...
                            $(dialog.fields_dict.field_checkboxes.wrapper)
                                .find(".field-row")
                                .each(function() {
                                    const fieldLabel = $(this).find(".field-label").text().toLowerCase();
            ...
            frappe.require('assets/frappe/js/lib/sortable.min.js', () => {
                new Sortable(dialog.fields_dict.field_checkboxes.$wrapper.find('#sortable-fields')[0], {
                    handle: '.drag-handle',
                    animation: 150,
            ...
            // On open: ensure required is disabled if readonly is checked
            $(dialog.fields_dict.field_checkboxes.wrapper).find('.field-row').each(function() {
            ...
            setTimeout(() => {
                $(dialog.fields_dict.field_checkboxes.wrapper).find('.field-include').on('change', function() {
                    const fieldname = $(this).data('fieldname');
                    const checked = $(this).is(':checked');
                    const $readonly = $(dialog.fields_dict.field_checkboxes.wrapper).find(`.field-readonly[data-fieldname='${fieldname}']`);
                    const $required = $(dialog.fields_dict.field_checkboxes.wrapper).find(`.field-required[data-fieldname='${fieldname}']`);
                ...
                $(dialog.fields_dict.field_checkboxes.wrapper).find('.field-readonly').on('change', function() {
                    const fieldname = $(this).data('fieldname');
                    const checked = $(this).is(':checked');
                    const $required = $(dialog.fields_dict.field_checkboxes.wrapper).find(`.field-required[data-fieldname='${fieldname}']`);
                    if (checked) {
                        ...
                        const $include = $(dialog.fields_dict.field_checkboxes.wrapper).find(`.field-include[data-fieldname='${fieldname}']`);
