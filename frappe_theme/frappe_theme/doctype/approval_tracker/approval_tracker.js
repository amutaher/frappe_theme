...
const show_table = async (frm, document_type) => {
    ...
    wf_field = await frappe.db.get_value('Workflow', { 'document_type': document_type, "is_active": 1 }, 'workflow_state_field')
            ...
            dt_events: {
                [document_type]: {
                    before_load: async function (dt) {
                        let wf_positive_closure = await frappe.xcall('frappe_theme.utils.get_wf_state_by_closure', { 'doctype': document_type, "closure_type": "Positive" })
                        let wf_negative_closure = await frappe.xcall('frappe_theme.utils.get_wf_state_by_closure', { 'doctype': document_type, "closure_type": "Negative" })
                    ...
                    after_workflow_action: async function (dt) {
                        try {
                            ...
                            if (dt.frm?.sva_dt_instance) {
                                dt.frm.sva_dt_instance.reloadTable()
                            }
                        } catch (error) {
