...
class CustomImporter:
	def __init__(
		...
		ref_doctype = self.data_import.custom_parent_ref_doctype
		ref_doc = self.data_import.custom_parent_ref_doc
...
class ImportFile:
	...
	def get_data_from_template_file(self):
		...
		if content:
			data = self.read_content(content, extension)
			# print(self.import_data())
			if self.ref_doctype in data[0]:
				return data
			else:
				
				cur_doc_field = frappe.get_list('DocField', filters={'parent': self.doctype,'fieldtype':'Link'}, fields=['fieldname','label','options'])
				docfields = frappe.get_list('DocField',
								 filters={'parent': self.ref_doctype,
				  					'fieldtype':'Link',
									'options':['in',[item.get('options') for item in cur_doc_field] ]
									}, fields=['fieldname','label'])
				for field in cur_doc_field:
					if field.get('options') == self.ref_doctype:
						field['fieldname'] = 'name'
						docfields.append(field)
				for field in docfields:
					for f in cur_doc_field:
						if field.options == f.options:
							field['label'] = f.label
					data[0].append(field.label)
				doc = frappe.get_doc(self.ref_doctype, self.ref_doc)
				for row in data[1:]:
					for field in docfields:
						row.append(doc.get(field.fieldname,''))
				return data
